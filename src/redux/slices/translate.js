...
export const translitarate = createSlice({
  ...
  reducers: {
    ...
    setStartAndEndIndex: (state, action) => {
      ...
      state.selectionStartIndex = !isNaN(action.payload[0])
        ? action.payload[0]
        : initialState.selectionStartIndex
      state.selectionEndIndex = !isNaN(action.payload[1])
        ? action.payload[1]
        : initialState.selectionEndIndex
    },
